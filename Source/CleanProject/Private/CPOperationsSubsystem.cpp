...
void FCPAssetDependenciesTable::BuildDependenciesTable(const TSet<FAssetData>& InAssets, EScanType ScanType) {
    ...
    for (FAssetData Asset : InAssets) {
        ...
        TArray<FString> Dependencies =
            UEditorAssetLibrary::FindPackageReferencersForAsset(AssetPath, bLoadAssetsToConfirm);
        ...
        UE_LOG(LogCleanProject, VeryVerbose, TEXT("Inspecting: %s took %s seconds"), *AssetPath,
               *LexToString(AssetTimer.GetTime()));
    }
...
void FCPAssetDependenciesTable::CompileReferencesRecursive(const TArray<FAssetData>& Assets,
                                                           TSet<FAssetData>& OutReferences,
                                                           int RecursionLevel /* = 0 */) {
    for (const FAssetData& Asset : Assets) {
        UE_LOG(LogCleanProject, VeryVerbose, TEXT("%s%s"), *GetTabSpaces(RecursionLevel),
...
TArray<FAssetData> UCPOperationsSubsystem::GetUnusedAssets(const TArray<FAssetData>& AssetsToCheck,
                                                           EScanType ScanType) const {
...
TSet<FAssetData> UCPOperationsSubsystem::GetAllCoreAssets() const {
    ...
    Algo::Transform(PackagingSettings->DirectoriesToAlwaysCook, FoldersToCook,
                    [](FDirectoryPath const& Directory) { return Directory.Path; });
...
void UCPOperationsSubsystem::ModifyCook(TConstArrayView<const ITargetPlatform*> InTargetPlatforms,
    ...
    for (const FAssetData& ExcludedAsset : AssetsExcludedFromPackage) {
        UE_LOG(LogCleanProject, Display,
               TEXT("%s is excluded from packaging and will not be included in the final build"),
